    ...
    $dataID = $db->escape(Input::get('dataID'));
        ...
        if(Input::get('action') == 'insert'){
          $validation = $validate->check($_GET, array(
            'sup_name' => array(
              'tag_name' => 'Supplier Name', 
              'required' => true,
              'unique' => 'tbl_supplier'
              ),
            'TIN' => array(
              'tag_name' => 'Tax Identification No.', 
              'required' => true,
              'unique' => 'tbl_supplier'
              )
          ));
  ...
  <section class="content">
    <div class="row">
      <div class="col-md-12">
        ...
        <!-- Default box -->
        <div class="box">
          <form class="regForm" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]) . (Input::get('sup_no') ?  "?action=update" : "") ?>" method="GET" >
             ...
             <tbody>
              ...
              while ($dataAttri = $sqlResult->fetch_assoc()) {
                ...
                <tr>
                  ...
                  <a href="/akempco/includes/delist.php?action=update&page=<?php echo basename($_SERVER['PHP_SELF']); ?>&table=<?php echo $table; ?>&filter=<?php echo $filter?>&dataID=<?php echo $dataID; ?>&active=0" onclick="return (confirm('Are you sure you want to delist this supplier?'));">DELIST</a></td>
