...
final class Mysqli_Manager extends \mysqli 
{
	...
	public function __construct() {
		...
		$this->db_connect = @parent::__construct(Config::get('mysql/host'), Config::get('mysql/username'), Config::get('mysql/password'), Config::get('mysql/db'));
    ...
    public static function getInstance(){
        if (!isset(self::$_instance)) {
            self::$_instance = new Mysqli_Manager();
        }
    ...
    */
    public function insert($into, array $array) {
		...
		$data = implode(', ', $data);
			...
			if ($this->query("INSERT INTO {$into} SET {$data}") ) {
                ...
                $this->_lastID = $this->insert_id;

			  	unset($into, $array, $data);

			  	$return = TRUE;
			}else{
                Session::flash('msg', $this->error);
            }
     ...
     */
 	public function multi_insert($into, array $array) {
        ...
        $val = array_filter($this->insert_ids);
		if (!empty($val)) {
			return TRUE; 
		}
