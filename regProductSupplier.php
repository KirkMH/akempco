...
require_once $_SERVER['DOCUMENT_ROOT'] . '/akempco/includes/sidebar.php';
  ...
  $filter = 'prod_no';
    ...
    if(Input::get('action') == 'insert') {//add
      ...
      if($db->delete($table, 'WHERE ps_no=' . Input::get('delID'))) {
        // Redirect::to($url);
      }else{
        echo "<script>alert('Opps! There was an error in saving in the database.')</script>";
      }
      ...
      <div class="col-md-12">
        ...
        <div class="box">
          ...
          <input class="button orange" type='hidden' name='token' value="<?php echo $token; ?>" />
            <div class="box-header with-border">
              <h3 class="box-title">Add New Supplier for this Product</h3>
              <div class="box-tools pull-right">
                <h5><a href="regProduct.php">View List of Products</a></h5>
              </div>                      
            </div>
            <div class="box-body">
              <div class="row">
                <div class="col-md-6">
                  ...
                  <input name="filter" value="<?php echo Input::get('filter')?>"  type="hidden">
        ...
        <div class="box">
          <div class="box-header">
            <h3 class="box-title">Supplier for this Product</h3>
            <div class="box-tools pull-right">
          ...
          <div class="box-body">
            <table id="listTable" class="sorttable table table-bordered table-striped">
              ...
              <tbody>
                <?php 
$sqlResult = $db->select('*',$table . " AS PS","INNER JOIN tbl_supplier AS S ON PS.sup_no = S.sup_no WHERE PS.prod_no=" . Input::get('dataID'));
                
                if($sqlResult){
                  while ($dataAttri = $sqlResult->fetch_assoc()) {
                    $delID = $dataAttri['sup_no'];
                  ?>
                    <tr>
                      <td>             
                      <a href="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>?filter=prod_no&dataID=<?php echo Input::get('dataID')?>&delID=<?php echo $delID; ?>">Remove</a></td> 
                        <td><?php echo $dataAttri['sup_name'];?></td>
                        <td><?php echo $dataAttri['sup_address'];?></td>
                        <td><?php echo $dataAttri['TIN'];?> </td>
                        <td><?php echo $dataAttri['bus_type'];?> </td>
                        <td><?php echo $dataAttri['tax_type'];?> </td>
                        <td><?php echo $dataAttri['contact_person'];?> </td>
